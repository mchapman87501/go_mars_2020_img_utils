...
func TestRGBToLab(t *testing.T) {
	...
	eq := func(a, b float64) bool {
		...
		fractDiff := absDiff / maxMag
		result := fractDiff <= epsilon
		if !result {
			pctDiff := float64(int(10000.0*fractDiff)) / 100
			fmt.Printf("  NOT EQUAL: %v, %v.  absDiff=%v, pctDiff=%.2f%%\n", a, b, absDiff, pctDiff)
		}
		return result
	}
	for _, tc := range labTestCases {
		labL, laba, labb := rgbToCIELab(tc.r, tc.g, tc.b)
		if !(eq(labL, tc.labL) && eq(laba, tc.laba) && eq(labb, tc.labb)) {
			t.Errorf(
